 ...
 */
import { GATEWAY_CONTRACT_VERSION, GATEWAY_CONTEXT_EXTENSION, GATEWAY_CONTEXT_PLACEHOLDER, GATEWAY_POLICY_VERSION, gateContextRequest, } from "../integrations/gateway-context-gate.js";
import { compile, gatewayValidate, verify, GATEWAY_NAMES } from "../context-control-cli/index.js";
...
const fail = (tool, code, message) => ({ ok: false, tool, error: { code, message }, boundary: EVIDENCE_BOUNDARY });
const succeed = (tool, result) => ({ ok: true, tool, result, boundary: EVIDENCE_BOUNDARY });
/**
...
export async function callMcpTool(name, args = {}, options = {}) {
    ...
    if (!tool)
        return fail(name, 'unknown_tool', `No such tool. Available: ${MCP_TOOLS.map((entry) => entry.name).join(', ')}`);
    ...
    switch (tool.name) {
        ...
        case 'context_control.validate_request': {
            ...
            return succeed(tool.name, gated.outcome === 'proceed'
                ? { outcome: 'proceed', limits: gated.limits }
                : gated);
        }
        ...
        case 'context_control.gateway_status': {
            ...
            if (typeof gateway !== 'string' || !GATEWAY_NAMES.includes(gateway)) {
                return fail(tool.name, 'invalid_arguments', `gateway must be one of ${GATEWAY_NAMES.join(', ')}`);
            }
            return succeed(tool.name, gatewayValidate(gateway, options.root ?? process.cwd()));
        }
    }
}
