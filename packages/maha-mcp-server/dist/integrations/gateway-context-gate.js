...
export function gatewayLimitsFrom(environment = process.env) {
 ...
 */
export function gatewaySecretFrom(environment = process.env) {
...
export function wholeDocumentContext(request) {
    return request.documents
        .map((document) => `[${document.id}] ${document.title ?? ''}\n${document.text}`)
        .join('\n\n');
}
export function replaceContextPlaceholder(messages, context) {
    let occurrences = 0;
 ...
 * separator is.
 */
export function evidenceHeaders(evidence) {
    return {
 ...
 */
export function gateContextRequest(input) {
    const limits = {
    ...
    if (!body)
        return { outcome: 'rejected', status: 400, code: 'invalid_envelope', message: 'The request body must be a JSON object.' };
    // Idempotence. Compiling twice would rewrite an already-rewritten prompt and
    // emit evidence describing the wrong input.
    if (input.alreadyCompiled)
        return { outcome: 'passthrough', reason: 'already_compiled' };
    if (body[GATEWAY_CONTEXT_EXTENSION] === undefined)
        return { outcome: 'passthrough', reason: 'no_context_extension' };
...
//# sourceMappingURL=gateway-context-gate.js.map