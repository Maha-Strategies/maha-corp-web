...
export async function instantiateKernel(bytes: BufferSource): Promise<WasmScienceKernel> {
  ...
  const required = ['normalizeAngleMicrodegrees', 'divideHalfEven', 'convertScaled', 'intervalAddLower', 'intervalAddUpper', 'integerSqrt', 'rootSumSquaresFloor', 'angularSeparationMicrodegrees', 'zodiacSignIndex', 'zodiacBoundaryDistanceMicrodegrees', 'layerThermalResistanceNanoKelvinPerWatt', 'temperatureRiseMicrokelvin'] as const
