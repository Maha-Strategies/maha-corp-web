...
import { createCalculationReceipt, } from './receipt.js';
export const EXECUTABLE_KERNEL_MODULE = '@maha/wasm-kernel';
export const EXECUTION_REQUEST_SCHEMA = 'maha-wasm-execution-request/1.0';
...
function integer(value, name) {
    if (!INTEGER.test(value))
        throw new Error(`${name} must be a canonical base-10 integer.`);
    const parsed = BigInt(value);
    if (parsed < BigInt('-9223372036854775808') || parsed > BigInt('9223372036854775807'))
        throw new Error(`${name} exceeds the signed-i64 range.`);
    return parsed;
}
function requireUnit(units, names, expected) {
    for (const name of names)
        if (units[name] !== expected)
            throw new Error(`${name} must use unit ${expected}.`);
}
export async function verifyKernelArtifact(artifact) {
    const findings = [];
    const manifest = artifact.manifest;
    if (manifest.schemaVersion !== 'maha-wasm-kernel-manifest/1.0' || manifest.abi !== 'wasm-i64-fixed-point')
        findings.push('kernel-manifest-schema-invalid');
    if (!manifest.conformanceVersion || !DIGEST.test(manifest.conformanceSha256) || !DIGEST.test(manifest.uncertaintyConformanceSha256))
        findings.push('kernel-conformance-identity-invalid');
    if (!DIGEST.test(manifest.kernelSha256) || manifest.kernelSha256 !== await digestBytes(artifact.bytes))
        findings.push('kernel-byte-digest-mismatch');
    if (manifest.compiler.name !== 'assemblyscript' || !manifest.compiler.version || !manifest.compiler.flags.length)
        findings.push('kernel-compiler-identity-invalid');
    if (manifest.arithmetic.integerModel !== 'signed-i64' || manifest.arithmetic.rounding !== 'nearest-ties-to-even' || manifest.arithmetic.overflow !== 'abort')
        findings.push('kernel-arithmetic-policy-invalid');
    try {
        await instantiateKernel(artifact.bytes);
    }
    catch {
        findings.push('kernel-instantiation-failed');
    }
    return [...new Set(findings)];
}
export async function executeKernelRequest(request, artifact) {
    const artifactFindings = await verifyKernelArtifact(artifact);
    if (artifactFindings.length)
        throw new Error(`Kernel artifact is invalid: ${artifactFindings.join(',')}`);
    if (request.schemaVersion !== EXECUTION_REQUEST_SCHEMA)
        throw new Error('Execution request schema is invalid.');
    if (Object.keys(request.constants ?? {}).length)
        throw new Error('This kernel version does not declare operation constants.');
    const kernel = await instantiateKernel(artifact.bytes);
    ...
    if (request.operation === 'layer-thermal-resistance-nanokelvin-per-watt') {
        ...
        const value = kernel.layerThermalResistanceNanoKelvinPerWatt(integer(request.inputs.thicknessNanometers, 'thicknessNanometers'), integer(request.inputs.areaSquareMicrometers, 'areaSquareMicrometers'), integer(request.inputs.conductivityMilliwattsPerMeterKelvin, 'conductivityMilliwattsPerMeterKelvin'));
    ...
    if (request.operation === 'interval-add') {
        ...
        if (!unit || Object.values(request.units).some((value) => value !== unit))
            throw new Error('Interval-add values must share one explicit unit.');
        const leftLower = integer(request.inputs.leftLower, 'leftLower');
        const leftUpper = integer(request.inputs.leftUpper, 'leftUpper');
        const rightLower = integer(request.inputs.rightLower, 'rightLower');
        const rightUpper = integer(request.inputs.rightUpper, 'rightUpper');
        if (leftLower > leftUpper || rightLower > rightUpper)
            throw new Error('Interval lower bounds must not exceed upper bounds.');
        const lower = kernel.intervalAddLower(leftLower, rightLower);
        const upper = kernel.intervalAddUpper(leftUpper, rightUpper);
...
export async function createExecutedCalculationReceipt(request, artifact) {
...
export async function verifyExecutedCalculationReceipt(receipt, artifact) {
    ...
    if (receipt.module !== EXECUTABLE_KERNEL_MODULE)
        findings.push('receipt-module-unsupported');
    if (receipt.kernelVersion !== artifact.manifest.kernelVersion || receipt.kernelSha256 !== artifact.manifest.kernelSha256)
        findings.push('receipt-kernel-identity-mismatch');
    if (receipt.conformanceVersion !== artifact.manifest.conformanceVersion || receipt.conformanceSha256 !== artifact.manifest.conformanceSha256)
        findings.push('receipt-conformance-identity-mismatch');
    if (JSON.stringify(receipt.compiler) !== JSON.stringify(artifact.manifest.compiler) || JSON.stringify(receipt.arithmetic) !== JSON.stringify(artifact.manifest.arithmetic))
    ...
    try {
        ...
        findings.push('receipt-execution-failed');
    }
    return [...new Set(findings)];
}
