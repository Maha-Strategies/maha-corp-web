-- Close the legacy-handoff bypass: every public release, including one from a
-- handoff created before fact-checking existed, needs a current acknowledged
-- review with no unresolved high-risk claims.
...
declare v_handoff public.content_publication_handoffs%rowtype; v_draft public.content_page_drafts%rowtype; v_candidate public.content_page_candidates%rowtype; v_review public.content_fact_check_reviews%rowtype; v_existing public.content_publications%rowtype;
begin
  ...
  select * into v_review from public.content_fact_check_reviews where draft_id=p_draft_id and superseded_at is null for update;
  if not found or v_review.high_risk_count <> 0 or v_review.acknowledged_at is null then raise exception 'An acknowledged fact-check review with no unresolved high-risk claims is required.' using errcode='P0001'; end if;
  ...
  insert into public.content_publication_events (publication_id,action,idempotency_hash,actor_fingerprint,note,created_at)
    values (p_publication_id,'published',p_idempotency_hash,p_actor_fingerprint,coalesce(nullif(p_note,''),'Human confirmed this public release after the score-qualified handoff and fact-check review.'),p_at);
