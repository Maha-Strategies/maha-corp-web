-- Persist the exact 27-record Batch 2 internal-review cohort as immutable draft
-- review targets. This function cannot create review decisions or releases.
...
alter table public.epistemic_ingestion_batches add constraint epistemic_ingestion_batches_adapter_id_check
  check (adapter_id in ('semiconductor','mathematics','astronomy','religion','neuromorphic-biocomputing','frontier-canary','substantial-batch-2-internal-review'));
...
create or replace function public.record_substantial_batch2_internal_review_targets(
  p_batch jsonb,
  p_records jsonb,
...
begin
  if p_batch is null or jsonb_typeof(p_batch) <> 'object'
    ...
    or coalesce(p_batch->>'adapterId','') <> 'substantial-batch-2-internal-review'
    or coalesce(p_batch->>'adapterVersion','') <> 'maha-epistemic-adapter/1.0'
    or coalesce(p_batch->>'sourceDatasetVersion','') <> 'maha-internal-review-batch-2/1.0'
    ...
    or coalesce(p_batch->>'recordCount','') <> '27'
    or jsonb_array_length(p_records) <> 27
    or jsonb_array_length(p_batch#>'{records}') <> 27
    or (select count(distinct record->>'sourceRecordId') from jsonb_array_elements(p_records) record) <> 27
    or p_idempotency_hash !~ '^sha256:[a-f0-9]{64}$'
    or p_actor_fingerprint !~ '^sha256:[a-f0-9]{64}$'
  then raise exception 'Invalid substantial Batch 2 internal-review target batch.' using errcode = '22023'; end if;
  ...
  if found then
    ...
    then raise exception 'Internal-review target idempotency cannot cross dataset revisions.' using errcode = 'P0001'; end if;
  ...
  insert into public.epistemic_ingestion_batches (
    ...
    p_batch->>'batchId', p_batch->>'schemaVersion', 'substantial-batch-2-internal-review', p_batch->>'adapterVersion', p_batch->>'sourceDatasetVersion',
    p_batch->>'sourceDatasetSha256', p_batch->>'batchSha256', 27, p_batch,
  ...
  for v_record in select value from jsonb_array_elements(p_records) loop
    if jsonb_typeof(v_record) <> 'object'
      ...
      or coalesce(v_record->>'adapterId','') <> 'substantial-batch-2-internal-review'
    ...
    then raise exception 'Invalid substantial Batch 2 internal-review target.' using errcode = '22023'; end if;
    ...
    insert into public.epistemic_ingestion_records (
      ...
      v_record->>'ingestionRecordId', p_batch->>'batchId', 'substantial-batch-2-internal-review', v_record->>'sourceRecordId', v_record->>'sourceRecordSha256',
  ...
  return jsonb_build_object('batchId', p_batch->>'batchId', 'recordCount', 27, 'idempotentReplay', false);
end; $$;

revoke all on function public.record_substantial_batch2_internal_review_targets(jsonb,jsonb,text,text) from public, anon, authenticated;
grant execute on function public.record_substantial_batch2_internal_review_targets(jsonb,jsonb,text,text) to service_role;

comment on function public.record_substantial_batch2_internal_review_targets(jsonb,jsonb,text,text) is
  'Persists only the frozen 27-record Batch 2 cohort as draft review targets. It cannot create review decisions, assert expert review, or publish records.';
