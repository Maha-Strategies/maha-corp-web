-- Freeze exactly the two repaired revisions as draft review targets. This RPC
-- cannot create decisions or releases and cannot be reused for another cohort.
create or replace function public.record_repaired_revision_canary_targets(
  p_batch jsonb,
  p_records jsonb,
...
begin
  if p_batch is null or jsonb_typeof(p_batch) <> 'object'
    ...
    or coalesce(p_batch->>'adapterId','') <> 'repaired-revision-canary'
    or coalesce(p_batch->>'adapterVersion','') <> 'maha-epistemic-adapter/1.0'
    or coalesce(p_batch->>'sourceDatasetVersion','') <> 'maha-repaired-revision-canary/1.0'
    ...
    or coalesce(p_batch->>'recordCount','') <> '2'
    or jsonb_array_length(p_records) <> 2
    or jsonb_array_length(p_batch#>'{records}') <> 2
    or (select count(distinct record->>'sourceRecordId') from jsonb_array_elements(p_records) record) <> 2
    or p_idempotency_hash !~ '^sha256:[a-f0-9]{64}$'
    or p_actor_fingerprint !~ '^sha256:[a-f0-9]{64}$'
  then raise exception 'Invalid repaired-revision canary target batch.' using errcode = '22023'; end if;
  ...
  if found then
    ...
    then raise exception 'Repaired-revision canary idempotency cannot cross dataset revisions.' using errcode = 'P0001'; end if;
  ...
  insert into public.epistemic_ingestion_batches (
    ...
    p_batch->>'batchId', p_batch->>'schemaVersion', 'repaired-revision-canary', p_batch->>'adapterVersion', p_batch->>'sourceDatasetVersion',
    p_batch->>'sourceDatasetSha256', p_batch->>'batchSha256', 2, p_batch,
  ...
  for v_record in select value from jsonb_array_elements(p_records) loop
    if jsonb_typeof(v_record) <> 'object'
      ...
      or coalesce(v_record->>'adapterId','') <> 'repaired-revision-canary'
    ...
    then raise exception 'Invalid repaired-revision canary target.' using errcode = '22023'; end if;
    ...
    insert into public.epistemic_ingestion_records (
      ...
      v_record->>'ingestionRecordId', p_batch->>'batchId', 'repaired-revision-canary', v_record->>'sourceRecordId', v_record->>'sourceRecordSha256',
  ...
  return jsonb_build_object('batchId', p_batch->>'batchId', 'recordCount', 2, 'idempotentReplay', false);
end; $$;

revoke all on function public.record_repaired_revision_canary_targets(jsonb,jsonb,text,text) from public, anon, authenticated;
grant execute on function public.record_repaired_revision_canary_targets(jsonb,jsonb,text,text) to service_role;

comment on function public.record_repaired_revision_canary_targets(jsonb,jsonb,text,text) is
  'Persists only the frozen two-record repaired-revision cohort as draft review targets. It cannot create review decisions or publish records.';
