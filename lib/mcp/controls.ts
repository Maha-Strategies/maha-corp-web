...
export class MCPControls {
  static async getPolicy(tenantId: string): Promise<MCPSlaPolicy> {
    const stored = await traceRedisQuery('GET', () => redis.get<string | MCPSlaPolicy>(policyKey(tenantId)))
  ...
  static async setPolicy(tenantId: string, value: unknown): Promise<MCPSlaPolicy> {
    ...
    await traceRedisQuery('SET', () => redis.set(policyKey(tenantId), JSON.stringify(policy)))
    return policy
  }
  ...
  static async consumeRateLimit(tenantId: string, limit: number): Promise<{ allowed: boolean; remaining: number; retryAfterSeconds: number }> {
    ...
    const result = await traceRedisQuery('EVAL', () => redis.eval(script, [key], [String(limit)])) as number[]
  ...
  static async beforeRequest(tenantId: string, serverId: string, policy: MCPSlaPolicy): Promise<{ allowed: boolean; retryAfterSeconds: number }> {
    ...
    const result = await traceRedisQuery('EVAL', () => redis.eval(script, [circuitKey(tenantId, serverId)], [String(now), String(policy.failureThreshold), String(policy.timeoutMs), String(policy.cooldownMs)])) as number[]
  ...
  static async recordSuccess(tenantId: string, serverId: string): Promise<void> {
    await traceRedisQuery('DEL', () => redis.del(circuitKey(tenantId, serverId)))
  }
  ...
  static async recordFailure(tenantId: string, serverId: string, policy: MCPSlaPolicy): Promise<void> {
    ...
    await traceRedisQuery('EVAL', () => redis.eval(script, [circuitKey(tenantId, serverId)], [String(policy.failureThreshold), String(now), String(policy.cooldownMs)]))
  }
}
