...
export function selectFederationTranche(
  ...
  trancheNumber: 3 | 4 | 5 | 6 | 7 | 8 | 9 | 10,
) {
  ...
  const priorIds = new Set(priorCohorts.flatMap((cohort) => cohort.entries.map((entry) => entry.candidateId)))
  ...
  const eligible = candidateMap.candidates.filter((candidate) =>
    semanticById.get(candidate.candidateId)?.disposition === 'retain-distinct'
    && !blockedByMissingOwner.has(candidate.candidateId)
    && !priorIds.has(candidate.candidateId))
  const eligibleIds = new Set(eligible.map((candidate) => candidate.candidateId))
  const ranked = [...eligible].sort((left, right) =>
    demandById.get(right.candidateId)!.calibratedScore - demandById.get(left.candidateId)!.calibratedScore
    || codeUnit(left.url, right.url))
  ...
  const closure = (id: string, accumulator = new Set<string>()): Set<string> => {
    ...
    if (!eligibleIds.has(id)) throw new Error(`Candidate ${id} depends on an unavailable candidate.`)
    accumulator.add(id)
  ...
  const entries = [...selected]
    ...
    .map((id, index) => {
      ...
      return {
        ...
        dependenciesSatisfiedByPriorTranches: allDependencies.filter((dependency) => priorIds.has(dependency)),
        evidenceRequirements: evidenceRequirements(candidate),
        evidenceState: 'not-inspected' as const,
        pageSpecEligible: false,
      }
    })
  const body = {
    schemaVersion: trancheNumber === 3
          ...
          : trancheNumber === 6
            ...
            : trancheNumber === 7
              ...
              : trancheNumber === 8
                ...
                : trancheNumber === 9
                  ? 'maha-federation-tranche-nine-cohort/1.0'
                  : 'maha-federation-tranche-ten-cohort/1.0',
          ...
          : trancheNumber === 6
            ...
            : trancheNumber === 7
              ...
              : trancheNumber === 8
                ...
                : trancheNumber === 9
                  ? 'Highest calibrated utility after Tranches 1 through 8, with dependency closure, a four-page property/topic cap, proportional property caps, and prior-tranche definitions treated as satisfied prerequisites.'
                  : 'Highest calibrated utility after Tranches 1 through 9, with dependency closure, a four-page property/topic cap, proportional property caps, and prior-tranche definitions treated as satisfied prerequisites.',
    propertyLimits: PROPERTY_LIMITS,
    counts: {
      selected: entries.length,
      overlapWithPriorTranches: entries.filter((entry) => priorIds.has(entry.candidateId)).length,
      dependenciesSatisfiedByPriorTranches: entries.filter((entry) => entry.dependenciesSatisfiedByPriorTranches.length > 0).length,
