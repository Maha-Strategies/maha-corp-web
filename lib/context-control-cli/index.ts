...
export type DoctorFinding = { check: string; status: 'ok' | 'warn' | 'fail'; detail: string }
export type DoctorReport = { status: 'ok' | 'incomplete'; findings: DoctorFinding[]; contractVersion: string }
 ...
 * what they need.
 */
export function doctor(environment: NodeJS.ProcessEnv = process.env): DoctorReport {
...
export async function compile(options: {
  ...
  try { JSON.parse(raw) } catch { throw new Error('The input file is not valid JSON.') }
...
export type VerifyFinding = { check: string; status: 'ok' | 'fail'; verifiable: 'locally' | 'trusted-passthrough'; detail: string }
export type VerifyReport = { status: 'ok' | 'invalid'; findings: VerifyFinding[] }
 ...
 * verified.
 */
export function verify(record: unknown): VerifyReport {
  const findings: VerifyFinding[] = []
  const add = (check: string, ok: boolean, verifiable: VerifyFinding['verifiable'], detail: string): void => {
  ...
  const headers = typeof value.headers === 'object' && value.headers !== null ? value.headers as Record<string, string> : {}
  ...
  add('policy-version', value.policyVersion === GATEWAY_POLICY_VERSION, 'locally',
    `expected ${GATEWAY_POLICY_VERSION}, found ${String(value.policyVersion)}`)
  add('retention-declared', value.sourceTextRetained === false && value.credentialsRetained === false, 'locally',
    'sourceTextRetained and credentialsRetained must both be false')
  ...
  if (outcome === 'compiled') {
    ...
    add('passthrough-consistency', headers[GATEWAY_COMPILED_HEADER] === undefined, 'locally',
      'a passthrough record must not claim x-maha-compiled')
  }
...
export type GatewayValidation = { gateway: GatewayName; status: 'ok' | 'invalid'; checks: { check: string; status: 'ok' | 'fail'; detail: string }[] }

const GATEWAY_ARTIFACTS: Record<GatewayName, { path: string; must: [string, RegExp][]; mustNot: [string, RegExp][] }> = {
  wso2: {
...
export function gatewayValidate(gateway: GatewayName, root = process.cwd()): GatewayValidation {
  const spec = GATEWAY_ARTIFACTS[gateway]
  const checks: GatewayValidation['checks'] = []
  let source = ''
  try {
...
export const GATEWAY_NAMES: readonly GatewayName[] = ['wso2', 'kong', 'apigee', 'cloudflare']
