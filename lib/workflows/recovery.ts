...
export interface RecoveryStore {
  ...
  list(tenantId: string, taskId: string): Promise<RecoveryRecord[]>
}
...
function indexKey(t: string, task: string) { return scopedRedisKey(`workflow:tenant:${digest(t)}:task:${digest(task)}:actions`) }
...
export class UpstashRecoveryStore implements RecoveryStore {
  async claim(input: Parameters<RecoveryStore['claim']>[0]) { validateBase(input); assertDigest(input.actionSha256, 'actionSha256'); assertDigest(input.policySha256, 'policySha256'); const claimedAt = input.claimedAt ?? new Date().toISOString(); const k = key(input.tenantId, input.taskId, input.actionId); const result = await traceRedisQuery('EVAL', () => redis.eval(CLAIM_LUA, [k, indexKey(input.tenantId, input.taskId)], [input.tenantId, input.taskId, input.actionId, input.actionSha256, input.policySha256, claimedAt, String(workflowRetentionSeconds())])) as Array<number>; const record = await this.get(input.tenantId, input.taskId, input.actionId); if (!record) throw new Error('Recovery claim was not durably recorded.'); if (record.actionSha256 !== input.actionSha256 || record.policySha256 !== input.policySha256) return { execute: false, record }; return { execute: Number(result[0]) === 1, record } }
  ...
  async list(t: string, task: string) { assertId(t, 'tenantId'); assertId(task, 'taskId'); const ids = await traceRedisQuery('SMEMBERS', () => redis.smembers(indexKey(t, task))) as string[]; const records = await Promise.all((ids ?? []).map((actionId) => this.get(t, task, actionId))); return records.filter((record): record is RecoveryRecord => record !== null).sort((left, right) => right.claimedAt.localeCompare(left.claimedAt)) }
}
export class MemoryRecoveryStore implements RecoveryStore {
  ...
  async list(t: string, task: string) { return [...this.records.values()].filter((record) => record.tenantId === t && record.taskId === task).sort((left, right) => right.claimedAt.localeCompare(left.claimedAt)).map((record) => ({ ...record })) }
}
