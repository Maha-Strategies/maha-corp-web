...
export function buildLlmsManifest(
  ...
  return [
    ...
    '- Authenticated epistemic factory MCP server: https://www.mahastrategies.com/api/mcp/epistemic-factory',
    '- Epistemic factory methodology and boundary: https://www.mahastrategies.com/knowledge/epistemic-system/publishing-factory',
    ...
    '- Tool availability is declared per server in mcp.json. The public preflight is callable and rate-limited; the epistemic factory is private, bearer-authenticated, and preview-only over MCP; the Context Control stdio server is source-available but its npm package is not published.',
    '- Durable factory queue submission and canonical release are separate admin operations. MCP tool arguments never accept credentials and cannot enqueue or publish.',
    ...
    '  - Research boundary: empirical, inferred and analogical claims remain explicitly separated. Similarity across scales is not presented as evidence of a repeated cosmic program.',
