...
export type BoundaryStatement = {
  ...
  transport: { kind: 'stdio' | 'http_loopback'; networkExposure: 'none' | 'loopback' }
...
export function isCredentialFieldName(key: string): boolean {
  ...
  for (let index = 0; index < parts.length - 1; index += 1) {
    if (CREDENTIAL_PAIRS.has(`${parts[index]} ${parts[index + 1]}`)) return true
  }
...
export function findCredentialFields(
  ...
  if (value && typeof value === 'object') {
    for (const [key, entry] of Object.entries(value)) {
      ...
      if (suspicious) found.push(`${path}.${key}`)
...
export function findUnboundedResponseStrings(value: unknown, path = '$', found: { path: string; length: number }[] = []) {
  if (typeof value === 'string') {
    if (value.length > MAX_RESPONSE_STRING_LENGTH) found.push({ path, length: value.length })
    return found
  }
  if (Array.isArray(value)) {
  ...
  if (value && typeof value === 'object') {
    for (const [key, entry] of Object.entries(value)) findUnboundedResponseStrings(entry, `${path}.${key}`, found)
  }
  return found
}
