...
export const AGENTCORE_ADAPTER_VERSION = '0.1.0' as const
...
export type ControlledCommerceErrorCode =
  | BuyerPolicyCode
...
function copyAudit(events: CommerceAuditEvent[]): CommerceAuditEvent[] {
  return events.map((event) => ({ ...event }))
}

/** Parses the only two fields a model is allowed to propose. */
export function parseAgentPurchaseArguments(value: string | unknown): AgentPurchaseRequest {
  let parsed: unknown = value
  if (typeof value === 'string') {
    try { parsed = JSON.parse(value) } catch { throw new Error('The x402_fetch arguments are not valid JSON.') }
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('The x402_fetch arguments must be an object.')
  const record = parsed as Record<string, unknown>
  const keys = Object.keys(record).sort()
  if (keys.length !== 2 || keys[0] !== 'purpose' || keys[1] !== 'resource_url') throw new Error('The model may provide only resource_url and purpose.')
  if (typeof record.resource_url !== 'string' || typeof record.purpose !== 'string') throw new Error('resource_url and purpose must be strings.')
  let resource: URL
  try { resource = new URL(record.resource_url) } catch { throw new Error('resource_url must be an absolute URL.') }
  if (resource.protocol !== 'https:' || resource.username || resource.password || resource.hash) throw new Error('resource_url must be public HTTPS without credentials or a fragment.')
  if (!PURPOSE.test(record.purpose)) throw new Error('purpose must be a bounded machine identifier.')
  return { resourceUrl: resource.toString(), purpose: record.purpose }
}

function isAuthorization(decision: PaymentPolicyDecision): decision is PaymentAuthorization {
  return decision.allowed === true
}

function isVerifiedSettlement(decision: SettlementDecision): decision is Extract<SettlementDecision, { verified: true }> {
 ...
 */
export function createAgentCoreControlledCommerceTool<Report>(config: AgentCoreControlledCommerceConfig<Report>) {
  let used = false

  return {
    async purchase(request: AgentPurchaseRequest, control: ApplicationPurchaseControl): Promise<AgentPurchaseEvidence<Report>> {
      const audit: CommerceAuditEvent[] = []
      const record = (eventType: CommerceAuditEventType, outcome: CommerceAuditEvent['outcome'], code?: string) => {
        audit.push({ sequence: audit.length + 1, eventType, outcome, ...(code ? { code } : {}) })
      }
      const fail = (code: ControlledCommerceErrorCode, message: string, cause?: unknown): never => {
        throw new ControlledCommerceError(code, message, copyAudit(audit), cause instanceof Error ? { cause } : undefined)
      }

      if (used) fail('access_count_invalid', 'This economic tool capability permits exactly one invocation.')
      used = true
      ...
      const challenge = await config.merchant.inspect(request.resourceUrl).catch((error: unknown) =>
        fail('merchant_delivery_failed', 'The merchant challenge could not be inspected.', error),
      )
      ...
      const authorization = decision as PaymentAuthorization
      record('policy_allowed', 'allowed')

      let session: AgentCorePaymentSession | null = null
      let completed: AgentPurchaseEvidence<Report> | null = null
      let failure: ControlledCommerceError | null = null
      ...
      try {
        session = await config.payments.createSession({
            ...
            fail('payment_session_invalid', 'The bounded payment session could not be created.', error),
          )
        ...
        const paid = await config.merchant.redeem({
            ...
            fail('merchant_delivery_failed', 'The paid merchant request failed.', error),
          )
        if (paid.status !== 200) fail('merchant_delivery_failed', `The paid merchant request returned HTTP ${paid.status}.`)
        record('merchant_accepted', 'completed')

        let onchain: OnchainSettlementEvidence
        try {
        ...
        if (!isVerifiedSettlement(settlement)) fail(settlement.code, settlement.message)
        const verified = settlement as Extract<SettlementDecision, { verified: true }>
      ...
      const evidence: AgentPurchaseEvidence<Report> = completed
