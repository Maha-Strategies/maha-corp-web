...
export const MICRO_RESULT_SCHEMAS: Record<MicroProductId, MicroSchema> = {
  ...
  'dimensional-consistency-check': obj({ dimensionOrder: arr(str(), 7, 7), actual: arr({ type: 'integer', minimum: -48, maximum: 48 }, 7, 7), expected: arr({ type: 'integer', minimum: -24, maximum: 24 }, 7, 7), consistent: bool, unitsConverted: no, formulaValidated: no }),
  'exact-interpolation-receipt': rational,
  'sampled-series-integration': rational,
  'evidence-conflict-comparator': obj({ state: en('insufficient-comparable-observations', 'conflict-observed', 'no-conflict-observed'), supporting: strings, opposing: strings, mixed: strings, excluded: arr(obj({ observationId: ID_SCHEMA, reasons: arr(en('population-mismatch', 'outcome-mismatch'), 1, 2) }), 0, 20), labelsIndependentlyVerified: no, consensusEstablished: no }),
  'release-bound-evidence-packet': obj({ canonicalUrl: str(512), title: str(1000), contentDigest: hash, release: obj({ releaseId: str(), releaseDigest: hash, status: en('active') }), review: obj({ exactRevisionReviewed: { type: 'boolean', enum: [true] }, basis: str(500), expertReviewClaimed: no }), sourceSnapshot: str(), sources: arr(source, 1, 64), inspectionPerformedThisCall: no, redistributionRightsGranted: no }),
  'tiruvaymoli-context-packet': obj({ canonicalUrl: str(512), registryDigest: hash, title: str(1000), passageRange: str(), voice: str(4000), place: str(4000), devotionalContext: str(4000), names: strings, identityRelationship: str(4000), notEstablished: str(4000), editions: arr(obj({ sourceId: str(), title: str(1000), publisher: str(1000), url: str(4096), version: str(4000), frame: en('primary-text-in-translation', 'attributed-scholarship'), inspectedLocator: str(4000), rightsBasis: str(4000), boundary: str(4000) }), 1, 8), answers: arr(obj({ question: str(4000), answer: str(12000) }), 5, 5), relatedUrls: strings, limitations: strings, sourceTextIncluded: no, redistributionRightsGranted: no, expertReviewClaimed: no }),
