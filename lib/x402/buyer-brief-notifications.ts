...
export function briefNoticeEmail(n:BriefNotice) {
  ...
  return {to:'mayone@mahastrategies.com',subject:`[Buyer-Brief] ${labels[n.kind]}: ${n.order_id}`,text:[
...
export async function buyerBriefNotificationsReady() {
  if(!process.env.RESEND_API_KEY || !process.env.CRON_SECRET || !process.env.BUYER_BRIEF_FROM_EMAIL)return false
...
export async function deliverBriefNotices() {
  const ledger=createAgentInquiryLedger(),key=process.env.RESEND_API_KEY,from=process.env.BUYER_BRIEF_FROM_EMAIL
