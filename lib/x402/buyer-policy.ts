...
export const BUYER_POLICY_SCHEMA_VERSION = '1.0.0' as const
...
export type BudgetReservationResult =
  | { reserved: true; spentAfter: string }
  | { reserved: false; reason: 'authorization_replayed' | 'task_limit_exceeded'; spentBefore: string }
...
export type SettlementDecision = VerifiedSettlement | (PolicyDenial & { verified: false })
...
function deny(code: Exclude<BuyerPolicyCode, 'allowed'>, message: string): PolicyDenial {
...
function normalizedIdentifier(value: string, network: string): string {
...
function assetRule(policy: BuyerPolicy, requirement: PaymentRequirementLike): BuyerAssetRule | null {
  return policy.assetRules.find((candidate) =>
    candidate.network === requirement.network
      && normalizedIdentifier(candidate.asset, candidate.network) === normalizedIdentifier(requirement.asset, requirement.network),
  ) ?? null
}

function validatePolicy(policy: BuyerPolicy): string | null {
  if (policy.schemaVersion !== BUYER_POLICY_SCHEMA_VERSION) return 'Unsupported buyer policy schemaVersion.'
  if (!BOUNDED_ID.test(policy.policyId) || !policy.policyVersion.trim()) return 'policyId or policyVersion is invalid.'
  if (policy.approvedSchemes.length === 0 || policy.approvedResources.length === 0 || policy.approvedPayees.length === 0 || policy.assetRules.length === 0) return 'Buyer policy allowlists cannot be empty.'
  if (policy.approvedResources.some((resource) => !canonicalResource(resource))) return 'Every approved resource must be an exact public HTTPS URL without credentials or fragments.'
  for (const rule of policy.assetRules) {
    ...
    if (!CAIP_2.test(rule.network) || !rule.asset || !call || !task || call > task) return 'Buyer policy asset limits are invalid.'
    if (rule.network.startsWith('eip155:') && !EVM_ADDRESS.test(rule.asset)) return 'An EVM asset must be a contract address.'
    if (rule.humanApprovalAbove !== undefined && (approval === null || approval > call)) return 'humanApprovalAbove must be non-negative and no greater than maxAmountPerCall.'
  }
  const assetRuleIds = policy.assetRules.map((rule) => `${rule.network}:${normalizedIdentifier(rule.asset, rule.network)}`)
  if (new Set(assetRuleIds).size !== assetRuleIds.length) return 'Buyer policy asset rules must be unique by network and asset.'
  if (typeof policy.settlement.requirePaymentResponse !== 'boolean' || typeof policy.settlement.requireOnchainConfirmation !== 'boolean') return 'Settlement evidence controls must be boolean.'
  if (!policy.settlement.requirePaymentResponse && !policy.settlement.requireOnchainConfirmation) return 'At least one settlement evidence control must be required.'
  return null
}

function approvalCovers(input: PaymentIntent, policy: BuyerPolicy, amount: bigint, now: Date): boolean {
...
export function evaluatePaymentIntent(policy: BuyerPolicy, input: PaymentIntent, now = new Date(), approvalAuthenticated = false): PaymentPolicyDecision {
  const policyError = validatePolicy(policy)
  if (policyError) return deny('invalid_policy', policyError)
  if (!BOUNDED_ID.test(input.taskId) || !BOUNDED_ID.test(input.authorizationId)) return deny('invalid_intent', 'taskId and authorizationId must be bounded stable identifiers.')
  ...
  if (!requestedResource || !declaredResource) return deny('invalid_intent', 'Requested and declared resources must be exact public HTTPS URLs.')
  if (!policy.approvedResources.map(canonicalResource).includes(requestedResource)) return deny('resource_not_approved', 'The requested resource is not allowlisted.')
  if (requestedResource !== declaredResource) return deny('resource_mismatch', 'The live challenge is bound to a different resource.')

  const requirement = input.requirement
  if (!policy.approvedSchemes.includes(requirement.scheme)) return deny('unsupported_scheme', 'The payment scheme is not approved.')
  ...
  if (requirement.network.startsWith('eip155:') && (!EVM_ADDRESS.test(requirement.payTo) || !EVM_ADDRESS.test(requirement.asset))) return deny('invalid_intent', 'The EVM asset or payee is malformed.')
  const payee = normalizedIdentifier(requirement.payTo, requirement.network)
  if (!policy.approvedPayees.some((candidate) => normalizedIdentifier(candidate, requirement.network) === payee)) return deny('payee_not_approved', 'The payee is not allowlisted.')

  const amount = parsePositiveInteger(requirement.amount)
  const callLimit = parsePositiveInteger(rule.maxAmountPerCall)!
  if (!amount) return deny('invalid_amount', 'The amount must be a positive integer in asset base units.')
  if (amount > callLimit) return deny('call_limit_exceeded', 'The payment exceeds the per-call ceiling.')
  if (policy.requireValidatedSchema && input.schema.status !== 'valid') return deny('schema_not_validated', 'A valid discovery schema is required before signing.')
  ...
  if (approvalThreshold !== null && amount > approvalThreshold) {
    if (!input.approval) return deny('human_approval_required', 'This amount requires scoped human approval.')
    if (!approvalCovers(input, policy, amount, now)) return deny('human_approval_invalid', 'The human approval is expired or does not bind every payment term.')
    if (!approvalAuthenticated) return deny('human_approval_required', 'The scoped approval must be authenticated by a trusted verifier.')
  }
...
export async function authorizePayment(input: {
  ...
  if (!decision.allowed && decision.code === 'human_approval_required' && input.intent.approval && input.verifyHumanApproval) {
    ...
    if (!authenticated) return deny('human_approval_invalid', 'The trusted approval verifier rejected this approval.')
  ...
  if (!reserved.reserved) {
    return deny(
      reserved.reason,
      reserved.reason === 'authorization_replayed'
...
function settlementDenial(code: Exclude<BuyerPolicyCode, 'allowed'>, message: string): SettlementDecision {
...
export function verifySettlement(input: {
  ...
  if (policy.settlement.requirePaymentResponse && !receipt) return settlementDenial('receipt_missing', 'PAYMENT-RESPONSE is required.')
  if (receipt) {
    if (receipt.success !== true) return settlementDenial('receipt_unsuccessful', 'The receipt does not report successful settlement.')
    if (receipt.network !== authorization.network) return settlementDenial('receipt_network_mismatch', 'The receipt network differs from the authorized network.')
    if (normalizedIdentifier(receipt.payer ?? '', authorization.network) !== normalizedIdentifier(payer, authorization.network)) return settlementDenial('receipt_payer_mismatch', 'The receipt payer differs from the signing wallet.')
    if (!TRANSACTION.test(receipt.transaction ?? '')) return settlementDenial('receipt_transaction_invalid', 'The receipt transaction identifier is missing or malformed.')
  }

  if (policy.settlement.requireOnchainConfirmation && !onchain) return settlementDenial('settlement_evidence_missing', 'Independent on-chain settlement evidence is required.')
  if (onchain?.status === 'indeterminate') return settlementDenial('settlement_indeterminate', `On-chain settlement is indeterminate: ${onchain.reason}`)
  if (onchain?.status === 'contradicted') return settlementDenial('settlement_contradicted', `On-chain evidence contradicts settlement: ${onchain.reason}`)
  if (onchain?.status === 'confirmed') {
    ...
    if (!identifiersMatch) return settlementDenial('settlement_mismatch', 'On-chain evidence does not bind to the authorized transaction terms.')
    const settledAmount = parsePositiveInteger(onchain.amount)
    if (!settledAmount || settledAmount < BigInt(authorization.amount)) return settlementDenial('settlement_underpaid', 'The confirmed transfer is smaller than the authorized amount.')
  }

  const transaction = receipt?.transaction ?? (onchain?.status === 'confirmed' ? onchain.transaction : undefined)
  if (!transaction) return settlementDenial('settlement_evidence_missing', 'No verifiable settlement transaction was supplied.')
...
export async function verifyAndRecordSettlement(input: {
  ...
  if (!decision.verified) return decision
 ...
 */
export function createInMemoryBuyerPolicyLedger(): BuyerPolicyLedger {
  const authorizations = new Set<string>()
  const settlements = new Set<string>()
  const taskSpend = new Map<string, bigint>()
  return {
    async reserve(input) {
      ...
      if (authorizations.has(authorizationKey)) return { reserved: false, reason: 'authorization_replayed', spentBefore: spentBefore.toString() }
      const spentAfter = spentBefore + BigInt(input.amount)
      if (spentAfter > BigInt(input.maxAmountPerTask)) return { reserved: false, reason: 'task_limit_exceeded', spentBefore: spentBefore.toString() }
    ...
    async claimSettlement(transaction) {
      ...
      if (settlements.has(key)) return false
      settlements.add(key)
      return true
    },
  }
