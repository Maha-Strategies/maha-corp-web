import { buildBookEditionReceipt } from './book-edition-product.ts'
...
export function bookEditionPost(bookId: MachineBookId, offer: X402Offer) {
  const handler = async (request: Request): Promise<Response> => {
    ...
    try {
      return Response.json(buildBookEditionReceipt(bookId, JSON.parse(raw)), { status: 200, headers: { 'Cache-Control': 'no-store' } })
    } catch (error) {
