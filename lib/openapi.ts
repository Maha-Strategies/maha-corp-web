  ...
  paths: {
    ...
    '/api/v1/context/budget-ladder': {
      post: {
        ...
        description: 'A deterministic five-run bundle priced at 0.012 USDC over x402 v2 on Base. It performs no model inference, source acquisition, or factual verification and stores neither source text nor compiled context.',
        requestBody: { required: true, content: { 'application/json': { schema: CONTEXT_BUDGET_LADDER_DISCOVERY.inputSchema } } },
        responses: { '201': { description: 'Five context packs and their budget comparison.', content: { 'application/json': { schema: CONTEXT_BUDGET_LADDER_DISCOVERY.outputSchema } } }, '400': errorResponse('Invalid ladder request.'), '402': errorResponse('The x402 payment challenge for 12000 USDC base units.'), '413': errorResponse('Request exceeds the published byte limit.'), '415': errorResponse('Content-Type must be application/json.') },
      },
    },
    ...
    '/api/v1/books/the-imagined-life/section': {
      post: {
        ...
        description: 'A deterministic section retrieval priced at 0.025 USDC over x402 v2 on Base. The public web edition remains free. Payment purchases structured delivery and an integrity receipt; it does not create exclusive access, transfer copyright, or include analysis or factual certification.',
        requestBody: { required: true, content: { 'application/json': { schema: IMAGINED_LIFE_SECTION_OFFER.discovery.inputSchema } } },
        responses: { '200': { description: 'Exact published section with edition metadata and deterministic receipt.', content: { 'application/json': { schema: IMAGINED_LIFE_SECTION_OFFER.discovery.outputSchema } } }, '400': errorResponse('Unknown section or invalid request.'), '402': errorResponse('The x402 payment challenge for 25000 USDC base units.'), '413': errorResponse('Request exceeds 1024 bytes.'), '415': errorResponse('Content-Type must be application/json.') },
      },
    },
    '/api/v1/books/the-volcanic-engine/section': {
      post: {
        ...
        description: 'A deterministic section retrieval priced at 0.030 USDC over x402 v2 on Base. The public web edition remains free. Payment purchases structured delivery and an integrity receipt; it does not create exclusive access, transfer copyright, or include analysis or factual certification.',
        requestBody: { required: true, content: { 'application/json': { schema: VOLCANIC_ENGINE_SECTION_OFFER.discovery.inputSchema } } },
        responses: { '200': { description: 'Exact published section with edition metadata and deterministic receipt.', content: { 'application/json': { schema: VOLCANIC_ENGINE_SECTION_OFFER.discovery.outputSchema } } }, '400': errorResponse('Unknown section or invalid request.'), '402': errorResponse('The x402 payment challenge for 30000 USDC base units.'), '413': errorResponse('Request exceeds 1024 bytes.'), '415': errorResponse('Content-Type must be application/json.') },
      },
    },
