...
const base = '/knowledge/astrology/workflows'
const pages = clearingGuidesForLane('astrology-infrastructure')
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!page) return {}
  return { metadataBase: new URL(MAHA_SITE_URL), title: `${page.title} | Astrology Infrastructure`, description: page.summary, alternates: { canonical: page.path }, openGraph: { type: 'article', title: page.title, description: page.summary, url: `${MAHA_SITE_URL}${page.path}` } }
}
export default async function AstrologyClearingPage({ params }: PageProps) {
  const { category, slug } = await params
  const page = getClearingGuide(`${base}/${category}/${slug}`)
  if (!page) notFound()
