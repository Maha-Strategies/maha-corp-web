...
export default async function TiruvaymoliAtlasTopicPage({ params }: PageProps) {
  ...
  return <main className="min-h-screen bg-[#09090b] px-6 py-16 text-zinc-300 selection:bg-fuchsia-300 selection:text-black sm:px-12"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, '\\u003c') }} /><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-12 grid gap-14 lg:grid-cols-[minmax(0,1fr)_320px]"><article>
      ...
      <section className="mt-14 grid gap-6 md:grid-cols-2"><div className="border border-zinc-800 p-6"><p className="font-mono text-[10px] uppercase tracking-widest text-fuchsia-300">Place and poetic voice</p><p className="mt-4 text-sm leading-7 text-zinc-300">{item.place}</p><p className="mt-4 text-sm leading-7 text-zinc-400">{item.voice}</p></div><div className="border border-zinc-800 p-6"><p className="font-mono text-[10px] uppercase tracking-widest text-fuchsia-300">Divine names and identity relationships</p><ul className="mt-4 flex flex-wrap gap-2">{item.names.map((name) => <li key={name} className="border border-zinc-700 px-3 py-2 text-xs text-zinc-300">{name}</li>)}</ul><p className="mt-4 text-xs leading-6 text-zinc-500">{item.identityRelationship}</p></div></section>

      <section className="mt-6 border border-zinc-800 p-6"><p className="font-mono text-[10px] uppercase tracking-widest text-fuchsia-300">Devotional context</p><p className="mt-4 text-sm leading-7 text-zinc-300">{item.devotionalContext}</p></section>
      ...
      <section className="mt-14"><p className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Evidence layers</p><h2 className="mt-3 text-3xl font-semibold text-white">Five kinds of authority remain separate</h2><dl className="mt-7 grid gap-4 sm:grid-cols-2"><div className="border border-zinc-800 p-5"><dt className="font-mono text-[9px] uppercase tracking-widest text-fuchsia-300">Primary text</dt><dd className="mt-3 text-sm leading-6 text-zinc-400">Pāsurams {item.range} identify the printed unit. This guide does not reproduce or claim to establish the critical Tamil wording.</dd></div><div className="border border-zinc-800 p-5"><dt className="font-mono text-[9px] uppercase tracking-widest text-fuchsia-300">Named translation</dt><dd className="mt-3 text-sm leading-6 text-zinc-400">Every passage observation is bounded to Kausalya Hart’s English rendering and retains the exact numbered locator.</dd></div><div className="border border-zinc-800 p-5"><dt className="font-mono text-[9px] uppercase tracking-widest text-amber-300">Scholarship</dt><dd className="mt-3 text-sm leading-6 text-zinc-400">Attributed scholarship supplies literary and reception context; it does not silently rewrite or certify the passage.</dd></div><div className="border border-zinc-800 p-5"><dt className="font-mono text-[9px] uppercase tracking-widest text-amber-300">Historical inference</dt><dd className="mt-3 text-sm leading-6 text-zinc-400">Dating, institutional continuity, social practice, and material history require independent evidence and are not inferred from this unit alone.</dd></div><div className="border border-zinc-800 p-5 sm:col-span-2"><dt className="font-mono text-[9px] uppercase tracking-widest text-rose-300">Theology</dt><dd className="mt-3 text-sm leading-6 text-zinc-400">Divine identities, promises, polemics, and supernatural acts are reported as claims made within the poem, not as externally certified facts.</dd></div></dl></section>
