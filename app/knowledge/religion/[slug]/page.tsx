...
export default async function ReligionConceptPage({ params }: PageProps) {
  ...
  return <main className="min-h-screen bg-[#09090b] px-6 py-16 text-zinc-300 selection:bg-teal-300 selection:text-black sm:px-12"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, '\\u003c') }} /><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-12 grid gap-14 lg:grid-cols-[minmax(0,1fr)_330px]"><article>
      ...
      {item.slug === 'textual-authority' && <section className="mt-6 border border-amber-900/60 bg-amber-950/10 p-6"><p className="font-mono text-[10px] uppercase tracking-widest text-amber-300">Passage-level application</p><h2 className="mt-3 text-2xl font-semibold text-white">Audit the answer layer by layer</h2><p className="mt-4 text-sm leading-6 text-zinc-400">The Tamil source atlas applies this contract to forty-eight exact questions, retaining each source role, locator, limitation, and cross-period non-inference.</p><Link href={TAMIL_SOURCE_ATLAS_PATH} className="mt-4 inline-block text-xs text-amber-300 underline underline-offset-4 hover:text-white">Open the evidence atlas →</Link></section>}
