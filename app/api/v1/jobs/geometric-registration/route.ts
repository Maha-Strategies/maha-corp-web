...
import { dispatchToWorker, enqueueGeometricRegistrationJob } from '@/lib/jobs/queue'
...
export async function POST(request: Request) {
  ...
  let outcome
  try {
    outcome = await enqueueGeometricRegistrationJob({ request: parsed, keyId, zeroDataRetention, callbackUrl: `${new URL(request.url).origin}/api/v1/jobs/webhook` })
