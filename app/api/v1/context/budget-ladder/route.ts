import { buildContextBudgetLadder } from '@/lib/x402/context-product-family'
import { CONTEXT_BUDGET_LADDER_OFFER } from '@/lib/x402/offers'
...
const handler = async (request: Request): Promise<Response> => {
  ...
  if (Buffer.byteLength(raw, 'utf8') > CONTEXT_BUDGET_LADDER_OFFER.maxRequestBytes) {
    return Response.json({ error: { code: 'payload_too_large', message: 'Request exceeds the published byte limit.' } }, { status: 413 })
  }
  try {
    return Response.json(buildContextBudgetLadder(JSON.parse(raw)), { status: 201, headers: { 'Cache-Control': 'no-store' } })
  } catch (error) {
...
const metered = async (request: Request) => {
  ...
  await recordOfferUsage({ offerId: CONTEXT_BUDGET_LADDER_OFFER.id, eventKind: 'invocation', status: response.status, discoverySource: discoverySourceFrom(request.headers) })
  return response
}
