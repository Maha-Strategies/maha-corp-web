import { buildEvidenceRetentionMatrix } from '@/lib/x402/context-product-family'
import { EVIDENCE_RETENTION_MATRIX_OFFER } from '@/lib/x402/offers'
...
const handler = async (request: Request): Promise<Response> => {
  ...
  if (Buffer.byteLength(raw, 'utf8') > EVIDENCE_RETENTION_MATRIX_OFFER.maxRequestBytes) {
    return Response.json({ error: { code: 'payload_too_large', message: 'Request exceeds the published byte limit.' } }, { status: 413 })
  }
  try {
    return Response.json(buildEvidenceRetentionMatrix(JSON.parse(raw)), { status: 201, headers: { 'Cache-Control': 'no-store' } })
  } catch (error) {
...
const metered = async (request: Request) => {
  ...
  await recordOfferUsage({ offerId: EVIDENCE_RETENTION_MATRIX_OFFER.id, eventKind: 'invocation', status: response.status, discoverySource: discoverySourceFrom(request.headers) })
  return response
}
