...
export function ApiCreditDashboard({ status }: { status?: string }) {
  ...
  function updateConnectedKey(key: string, nextBalance: Balance) { setApiKey(key); setBalance(nextBalance); setError(null) }
  function clearConnectedKey() { setApiKey(''); setBalance(null); setError(null) }
  return <main className="min-h-screen bg-gray-50 px-6 py-14 text-gray-950 sm:py-20"><div className="mx-auto max-w-3xl">
    ...
    {(status === 'success' || status === 'subscription-success') && <div role="status" className="mt-8 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">Payment completed. Credits and subscription status appear after Stripe confirms the signed webhook.</div>}
