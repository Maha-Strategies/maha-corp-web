...
export default function ClaimLevelProvenancePage() {
  return <main className="min-h-screen bg-[#0a0a0c] px-6 py-20 text-zinc-300 sm:py-28"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, '\\u003c') }} />
    <article className="mx-auto max-w-4xl">
      ...
      <h1 className="mt-5 max-w-3xl text-4xl font-light leading-tight tracking-tight text-white sm:text-6xl">What is claim-level provenance?</h1>
      <p className="mt-7 max-w-3xl text-xl leading-relaxed text-zinc-300"><strong>Claim-level provenance is the record that stays attached to a substantive assertion:</strong> what it says, where it came from, how it is framed, what it does not establish, and when it was last checked.</p>

      <section className="mt-14 space-y-5 text-lg leading-relaxed text-zinc-400"><p>A bibliography can show that a document contains sources. It cannot, by itself, show which source supports which sentence, whether that sentence is an observation or an interpretation, or whether the author has verified the source rather than repeated it.</p><p>That distinction matters more when text is copied into a brief, quoted in a slide, or supplied as context to an AI system. A sentence often travels without its footnote, caveat, or surrounding disagreement. Claim-level provenance treats those qualifications as part of the claim record rather than decoration around it.</p></section>

      <section className="mt-14"><h2 className="text-2xl text-white">The minimum useful record</h2><div className="mt-6 divide-y divide-zinc-800 border-y border-zinc-800">{record.map(([label, description]) => <div key={label} className="grid gap-2 py-5 md:grid-cols-[10rem_1fr]"><h3 className="font-mono text-xs uppercase tracking-widest text-indigo-300">{label}</h3><p className="leading-relaxed text-zinc-400">{description}</p></div>)}</div></section>

      <section className="mt-14 grid gap-8 border-y border-zinc-800 py-10 md:grid-cols-2"><div><h2 className="text-2xl text-white">A concrete example</h2><p className="mt-4 leading-relaxed text-zinc-400">The De Sitter Atlas gives each concise statement a stable identifier, a status label, an explanation, a limitation, and source IDs. A reader can inspect a single claim instead of treating a whole page as equally certain.</p><a className="mt-5 inline-block text-zinc-100 underline underline-offset-4 hover:text-white" href="https://research.mahastrategies.com/atlas/de-sitter-swampland/claims/ds-001">Inspect atlas claim ds-001 ↗</a></div><div><h2 className="text-2xl text-white">What it does not do</h2><p className="mt-4 leading-relaxed text-zinc-400">A well-formed record does not establish truth. It makes a claim easier to challenge, update, and reuse responsibly. The primary source may still be wrong, incomplete, contested, or misapplied.</p></div></section>

      <section className="mt-14"><h2 className="text-2xl text-white">Use it in practice</h2><ol className="mt-5 list-decimal space-y-3 pl-6 leading-relaxed text-zinc-400"><li>Write the claim in a form that can be checked.</li><li>Name its epistemic status before expanding its rhetoric.</li><li>Link the best available source or explicitly name the source gap.</li><li>State the boundary: what a reader must not infer from it.</li><li>Publish a revision trail when the wording or evidence changes.</li></ol></section>

      <MpsLearningLinks current="/mps/claim-level-provenance" />
      <section className="mt-10 flex flex-wrap gap-4"><Link href="/audit" className="border border-zinc-600 px-5 py-3 font-mono text-xs uppercase tracking-widest text-zinc-100 hover:border-white">Try the free Auditor</Link><Link href="/mps" className="border border-zinc-600 px-5 py-3 font-mono text-xs uppercase tracking-widest text-zinc-100 hover:border-white">Read MPS/0.1</Link></section>
    </article>
  </main>
}
