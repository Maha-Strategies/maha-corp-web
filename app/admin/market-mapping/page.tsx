...
type SearchInsight = {
  id: string; kind: 'momentum' | 'near_page_one' | 'low_ctr'; query: string; priority: number; recommendation: string
  current: { clicks: number; impressions: number; ctr: number; position: number }; previous?: { impressions: number }
}
...
export default function MarketMappingPage() {
  ...
  const [latestSearchSnapshot, setLatestSearchSnapshot] = useState<string | null>(null)
  ...
  async function load() {
    ...
    try {
      ...
      setUnlocked(true)
      const performance = await fetch('/api/admin/search-performance', { headers: { Authorization: `Bearer ${token}` } })
      if (performance.ok) {
        const performanceBody = await performance.json() as { latestObservedOn?: string | null; insights?: SearchInsight[] }
        setLatestSearchSnapshot(performanceBody.latestObservedOn ?? null)
        setInsights(performanceBody.insights ?? [])
      } else { setLatestSearchSnapshot(null); setInsights([]) }
  ...
  return <main className="min-h-screen bg-[#0a0a0c] px-6 py-12 text-zinc-200"><div className="mx-auto max-w-6xl">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-mono text-xs tracking-widest text-cyan-300">[ MARKET MAPPING // PRIVATE ]</p><h1 className="mt-3 text-3xl text-white">Opportunity queue</h1><p className="mt-2 text-sm text-zinc-400">Evidence → deterministic score → human approval. No autonomous publishing, spend, deployment, or outreach.</p></div><div className="flex flex-wrap gap-3"><a href="/admin/search-performance" className="border border-cyan-700 px-4 py-2 font-mono text-xs uppercase text-cyan-100">Search performance</a><a href="/admin/content-workflow" className="border border-cyan-700 px-4 py-2 font-mono text-xs uppercase text-cyan-100">Content workflow</a><a href="/admin/experiments" className="border border-cyan-700 px-4 py-2 font-mono text-xs uppercase text-cyan-100">Experiments</a><select value={filter} onChange={(event) => setFilter(event.target.value as 'active' | 'all')} className="border border-zinc-700 bg-black p-2 text-sm"><option value="active">Active</option><option value="all">All</option></select><input aria-label="Search Console observation date" type="date" value={observedAt} onChange={(event) => setObservedAt(event.target.value)} className="border border-zinc-700 bg-black p-2 text-sm" /><input ref={fileInput} type="file" accept=".csv,text/csv" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ''; if (file) void importSearchConsole(file) }} /><button onClick={() => fileInput.current?.click()} disabled={loading} className="border border-cyan-700 px-4 py-2 font-mono text-xs uppercase text-cyan-100 disabled:opacity-40">Import GSC queries</button><button onClick={() => void runScout()} disabled={loading} className="border border-cyan-400 px-4 py-2 font-mono text-xs uppercase text-cyan-200 disabled:opacity-40">{loading ? 'Working…' : 'Run Scout · max 5'}</button><button onClick={() => void load()} disabled={loading} className="border border-zinc-600 px-4 py-2 font-mono text-xs uppercase">Refresh</button></div></div>
    {notice && <p className="mt-5 border border-red-800 bg-red-950/30 p-3 text-sm text-red-200">{notice}</p>}
    <section className="mt-8 border border-zinc-800 bg-zinc-950/50 p-5"><div className="flex flex-wrap items-baseline justify-between gap-3"><div><p className="font-mono text-[10px] uppercase tracking-widest text-cyan-300">Search performance feedback</p><h2 className="mt-2 text-lg text-white">Human-review content signals</h2><p className="mt-1 text-sm text-zinc-400">{latestSearchSnapshot ? `Latest imported Queries.csv: ${latestSearchSnapshot}. These are recommendations, not publishing actions.` : 'Apply the Search Performance Feedback migration, then import Queries.csv on two or more dates to establish a baseline.'}</p></div>{latestSearchSnapshot && <p className="font-mono text-xs text-zinc-500">{insights.length} actionable</p>}</div>{insights.length > 0 && <div className="mt-5 space-y-3">{insights.map((insight) => <article key={insight.id} className="border border-zinc-800 p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-mono text-[10px] uppercase tracking-widest text-cyan-300">{insight.kind.replaceAll('_', ' ')}</p><h3 className="mt-1 text-base text-white">{insight.query}</h3></div><p className="font-mono text-lg text-cyan-200">{insight.priority}</p></div><p className="mt-3 text-sm leading-relaxed text-zinc-300">{insight.recommendation}</p><p className="mt-3 font-mono text-[10px] uppercase tracking-widest text-zinc-500">{insight.current.impressions} impressions · {insight.current.clicks} clicks · {insight.current.ctr}% CTR · position {insight.current.position}</p></article>)}</div>}</section>
