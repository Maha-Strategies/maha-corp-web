...
export default function NavigatorResearchPage() {
  ...
  async function load() {
    ...
    try {
      const response = await fetch('/api/admin/navigator/research', { headers: { Authorization: `Bearer ${token}` } })
      ...
      setRubric(body.rubric ?? null); setCandidates(body.candidates ?? []); setGate(body.qualityGate ?? null); setUnlocked(true)
