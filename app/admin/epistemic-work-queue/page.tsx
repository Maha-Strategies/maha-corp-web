...
type EvidenceDraft = { sourceUrl: string; exactLocator: string; note: string; rightsBasis: string }
...
export default function EpistemicWorkQueuePage() {
  ...
  function chooseSource(item: SourceItem) {
    ...
    setEvidence(Object.fromEntries(item.blockers.map((blocker) => [blocker.code, { sourceUrl: '', exactLocator: '', note: '', rightsBasis: '' }])))
    setNote('')
  }
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <section className={styles.workspace}>
      ...
      <div className={styles.detailGrid}>{lane === 'source' ? selectedSource ? <>
        <section className={styles.panel}><p className={styles.kicker}>Frozen source target</p><h2 className={styles.sectionTitle}>{selectedSource.title}</h2><p className={`${styles.mono} mt-3`}>{selectedSource.recordId}<br />{selectedSource.targetSha256}</p><div className={styles.badges}><span className={priorityClass(selectedSource.priority)}>{selectedSource.priority}</span><span className={`${styles.badge} ${styles.normal}`}>{selectedSource.state.replaceAll('-', ' ')}</span><span className={`${styles.badge} ${styles.low}`}>{selectedSource.evidenceCount} evidence rows</span></div><a className={styles.reviewLink} href={selectedSource.sourcePublicPath}>Inspect legacy source page</a></section>
