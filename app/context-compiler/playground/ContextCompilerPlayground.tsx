  ...
  return (
    <main className="evidence-page">
      <div className="evidence-container">
        ...
        <section className="mt-12 border border-[var(--border-default)] bg-[var(--surface-raised)] p-5 sm:p-7" aria-labelledby="workload-heading">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="flex-1">
              ...
              <textarea id="task" value={task} onChange={(event) => setTask(event.target.value)} rows={3} className="mt-2 w-full border border-[var(--border-strong)] bg-[var(--surface-paper)] px-4 py-3 text-sm leading-6 text-[var(--text-primary)] focus:border-cyan-500 focus:outline-none" />
            </div>
            <div className="grid gap-4 sm:grid-cols-2 lg:w-[26rem]">
              <label className="block font-mono text-[10px] uppercase tracking-widest text-[var(--text-muted)]">Token budget
                <input type="number" min={64} max={16000} step={256} value={tokenBudget} onChange={(event) => setTokenBudget(Number(event.target.value))} className="mt-2 block w-full border border-[var(--border-strong)] bg-[var(--surface-paper)] px-4 py-3 text-sm text-[var(--text-primary)] focus:border-cyan-500 focus:outline-none" />
              </label>
              <button type="button" onClick={() => void runSample()} disabled={runState === 'loading' || runState === 'running'} className="self-end border border-cyan-500 bg-cyan-950/30 px-5 py-3 font-mono text-xs uppercase tracking-widest text-[var(--status-sourced)] hover:bg-cyan-900/40 disabled:cursor-wait disabled:opacity-50">
        ...
        {data && <>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-2" aria-labelledby="comparison-heading">
            ...
            <article className="min-w-0 border border-[var(--border-default)] bg-[var(--surface-raised)]">
              <div className="border-b border-[var(--border-default)] p-5">
                ...
                <select id="source" value={selectedSource} onChange={(event) => setSelectedSource(event.target.value)} className="mt-4 w-full border border-[var(--border-strong)] bg-[var(--surface-paper)] px-3 py-2 text-sm text-[var(--text-primary)]">
            ...
            <article className="min-w-0 border border-cyan-900/70 bg-cyan-950/10">
              <div className="border-b border-cyan-900/70 p-5">
                <p className="font-mono text-[10px] uppercase tracking-widest text-[var(--status-sourced)]">Compiled Context Pack</p>
          ...
          <section className="mt-6 border border-[var(--border-default)] p-5 sm:p-7" aria-labelledby="provenance-heading">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div><p className="font-mono text-[10px] uppercase tracking-widest text-[var(--status-sourced)]">Evidence path</p><h2 id="provenance-heading" className="mt-2 text-2xl text-[var(--text-primary)]">Retained passages from {activeSource?.title}</h2></div>
              <p className="font-mono text-xs text-[var(--text-muted)]">{retainedForSource.length} retained</p>
            </div>
            <div className="mt-6 grid gap-3 lg:grid-cols-2">
              {retainedForSource.map((passage) => <article key={passage.passageId} className="border-l-2 border-cyan-600 bg-cyan-950/10 p-4"><p className="font-mono text-[10px] uppercase tracking-widest text-[var(--status-sourced)]">{passage.passageId}</p><p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">{passage.text}</p><p className="mt-3 break-all font-mono text-[9px] text-[var(--text-muted)]">{passage.passageHash}</p></article>)}
            </div>
          </section>
          ...
          <section className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <article className="border border-[var(--border-default)] p-5 sm:p-7">
              ...
              <label className="mt-6 block max-w-xs font-mono text-[10px] uppercase tracking-widest text-[var(--text-muted)]">Model input price ($ / 1M tokens)
                <input type="number" min={0} step={0.1} value={modelPrice} onChange={(event) => setModelPrice(Math.max(0, Number(event.target.value)))} className="mt-2 block w-full border border-[var(--border-strong)] bg-[var(--surface-paper)] px-4 py-3 text-sm text-[var(--text-primary)]" />
              </label>
          ...
          <section className="mt-6 border border-[var(--border-default)] bg-[var(--surface-raised)] p-5 sm:p-7" aria-labelledby="integration-heading">
            <p className="font-mono text-[10px] uppercase tracking-widest text-[var(--status-sourced)]">Take it with you</p>
            <div className="mt-2 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 id="integration-heading" className="text-2xl text-[var(--text-primary)]">Copy a working integration</h2><p className="mt-2 text-sm text-[var(--text-secondary)]">Use an API key in application code. The browser payment above is deliberately human-authorized.</p></div><button type="button" onClick={() => void copyCode()} className="border border-[var(--border-strong)] px-4 py-2 font-mono text-xs uppercase tracking-widest text-[var(--text-primary)] hover:border-cyan-500">{copied ? 'Copied' : 'Copy code'}</button></div>
            <div className="mt-6 flex flex-wrap gap-2" role="tablist" aria-label="Integration language">
              {(['typescript', 'python', 'crewai', 'langchain'] as const).map((tab) => <button key={tab} type="button" role="tab" aria-selected={codeTab === tab} onClick={() => setCodeTab(tab)} className={`border px-4 py-2 font-mono text-[10px] uppercase tracking-widest ${codeTab === tab ? 'border-cyan-500 bg-cyan-950/30 text-[var(--status-sourced)]' : 'border-[var(--border-default)] text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}>{tab}</button>)}
            </div>
            <pre className="mt-4 max-h-[32rem] overflow-auto border border-[var(--border-default)] bg-[#141816] p-4 font-mono text-xs leading-6 text-[#f5f5ef]"><code>{snippets?.[codeTab]}</code></pre>
          </section>

          <section className="mt-6 border border-[var(--border-subtle)] p-5 text-xs leading-6 text-[var(--text-muted)]">
...
function Metric({ label, value, detail }: { label: string; value: string; detail: string }) {
  return <article className="border border-[var(--border-default)] bg-[var(--surface-raised)] p-5"><p className="font-mono text-[10px] uppercase tracking-widest text-[var(--status-sourced)]">{label}</p><p className="mt-3 font-mono text-2xl text-[var(--text-primary)]">{value}</p><p className="mt-3 text-xs leading-5 text-[var(--text-muted)]">{detail}</p></article>
}
