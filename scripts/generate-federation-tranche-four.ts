...
export function generateTrancheFour(outputRoot = ROOT) {
  ...
  const priorCohorts = [1, 2, 3].map((tranche) => readJson<Parameters<typeof selectFederationTranche>[4][number]>(`content/federation/federation-tranche-${tranche}-cohort-v1.json`))
  ...
  const cohort = selectFederationTranche(candidateMap, semantic, graph, demand, priorCohorts, 4)
  const output = resolve(outputRoot, 'content/federation/federation-tranche-4-cohort-v1.json')
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  const cohort = generateTrancheFour()
