...
const args = process.argv.slice(2), target = resolve(root, 'content/discovery/micro-next12-cost-observation-v5.json')
...
if (args[0] === '--check') {
  ...
  const body = { version: 'micro-next12-cost-observation/5', supersedes: 'micro-next12-cost-observation-v4.json', revisionReason: 'Recapture after the owner-approved 2026-09-13 seven-utility repricing. Preserve v4 as historical evidence; receipt prices and the contracts implementation digest changed.', basis: 'local-process; capped high-size fixtures, not an exhaustive worst-case proof or Vercel bill', observedAt: new Date().toISOString(), implementationFiles, implementationDigest, workloadsDigest: microDigest(workloads), nodeVersion: process.version, observations }
