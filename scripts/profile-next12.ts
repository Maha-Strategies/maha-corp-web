...
const args = process.argv.slice(2), target = resolve(root, 'content/discovery/micro-next12-cost-observation-v6.json')
...
if (args[0] === '--check') {
  ...
  const body = { version: 'micro-next12-cost-observation/6', supersedes: 'micro-next12-cost-observation-v5.json', revisionReason: 'Recapture after two withheld compatibility products extended the shared dispatcher and schemas. Existing twelve prices and workloads unchanged; preserve v1-v5 as historical evidence.', basis: 'local-process; capped high-size fixtures, not an exhaustive worst-case proof or Vercel bill', observedAt: new Date().toISOString(), implementationFiles, implementationDigest, workloadsDigest: microDigest(workloads), nodeVersion: process.version, observations }
