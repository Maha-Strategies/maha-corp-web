...
import { buildTrancheFiveReview } from '../lib/federation-tranche-five-review.ts'
...
export function generateFederationTrancheFiveReview(outputRoot: string) {
  type Input = Parameters<typeof buildTrancheFiveReview>[0]
  const cohort = verified<Input['cohort']>('content/federation/federation-tranche-5-cohort-v1.json')
  ...
  const result = buildTrancheFiveReview({ cohort, candidates, priorPacketManifests })
  const artifacts: Array<[string, unknown]> = [
    ['content/federation/federation-tranche-5-semantic-validation-v1.json', result.semanticManifest],
    ['content/federation/federation-tranche-5-dependency-validation-v1.json', result.dependencyManifest],
    ['content/federation/federation-tranche-5-evidence-packets-v1.json', result.packetManifest],
    ['content/federation/federation-tranche-5-decisions-v1.json', result.decisionManifest],
    ['content/federation/federation-tranche-5-page-specifications-v1.json', result.specificationManifest],
    ['content/federation/federation-tranche-5-readiness-v1.json', result.readiness],
  ]
  ...
  const reportPath = 'docs/federation/federation-candidates-401-500-local-readiness.md'
  const report = `${[
    '# Federation candidates 401–500 local readiness',
    '',
    ...
    '## Evidence boundaries',
    '',
    '- Commercialization pages remain revise unless a current exact offer establishes availability, price, scope, delivery, and customer-validation boundaries.',
    '- Metadata, identifiers, repository locations, and structured locators support discovery and identity only at their recorded depth; none is silently promoted to passage-level evidence.',
    '- Authorial concepts establish what the author proposes, not priority, independent reception, empirical validation, or consensus.',
    '- Biometric and algorithmic-impact pages are jurisdiction-specific information, not universal law or legal advice.',
    '- Seeds, uncertainty bounds, receipts, quotas, and replay controls prove only their bounded technical contracts; they do not prove correctness or production operation.',
    '',
    '## Build boundary',
    '',
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  const result = generateFederationTrancheFiveReview(resolve(argument('output-root') ?? ROOT))
