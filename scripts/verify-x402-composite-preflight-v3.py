#!/usr/bin/env python3
"""Verify the frozen NS Goods composite preflight v3 fixtures offline.
...
EIP-191 signatures, consumer-side invariants, tamper refusal, and v1/v2 rejection.
"""
...
ARCHIVE_ROOT = ROOT / "fixtures" / "x402-composite-preflight-v3"
FIXTURE_ROOT = ARCHIVE_ROOT / "fixtures"
V2_ROOT = ROOT / "fixtures" / "x402-composite-preflight-v2"
V1_ROOT = V2_ROOT / "superseded-v1"
...
def verify_manifest() -> dict[str, Any]:
    ...
    expect(service.get("schema") == "preflight_v3 (supersedes preflight_v2 at an announced version boundary, 2026-09-01)", "unexpected schema boundary")
    return {
...
def run_tamper_tests(validator: Draft202012Validator, clean: dict[str, Any]) -> dict[str, Any]:
    ...
    refusal_unknown_field_rejections = 0
    ...
    for filename, (status, reason) in REFUSAL_FIXTURES.items():
        ...
        cases += 1
        refusal = load_json(FIXTURE_ROOT / filename)
        refusal["unexpected"] = True
        try:
            validate_schema(validator, refusal, f"{filename}-unknown-field")
        except Exception:
            refusal_unknown_field_rejections += 1
        cases += 1
    expect(
        refusal_schema_rejections == len(REFUSAL_FIXTURES),
        "provider schema accepted an envelope injected into a refusal branch",
    )
    expect(
        refusal_unknown_field_rejections == len(REFUSAL_FIXTURES),
        "provider schema accepted an unknown field in a refusal branch",
    )
    expect(
    ...
    return {
        ...
        "refusalUnknownFieldsRejectedBySchema": refusal_unknown_field_rejections,
...
def run_audit() -> dict[str, Any]:
    ...
    schema = load_json(ARCHIVE_ROOT / "preflight_v3.schema.json")
    ...
    for filename in V1_FIXTURES:
        ...
        if list(validator.iter_errors(value)):
            rejected_v1 += 1
    expect(rejected_v1 == len(V1_FIXTURES), "v3 schema did not reject every archived v1 fixture")

    rejected_v2 = 0
    for filename in V2_FIXTURES:
        value = load_json(V2_ROOT / "fixtures" / filename)
        if list(validator.iter_errors(value)):
            rejected_v2 += 1
    expect(rejected_v2 == len(V2_FIXTURES), "v3 schema did not reject every archived v2 fixture")
    ...
    return {
        ...
        "provider": "NS Goods",
        "sourceUrls": {
            "contract": "https://x402.nsgoods.org/preflight/schema/preflight_v3.html",
            "schema": "https://x402.nsgoods.org/preflight/schema/preflight_v3.schema.json",
            "fixtureDigest": "https://x402.nsgoods.org/preflight/fixtures/preflight_v3/DIGEST",
            "proofManifest": MANIFEST_URL,
        },
        "authorizedCompositeSigner": EXPECTED_SIGNER,
        "artifacts": {
            "schemaSha256": digest_result["files"]["../preflight_v3.schema.json"],
            ...
            "documentationSha256": sha256_bytes((ARCHIVE_ROOT / "preflight_v3.html").read_bytes()),
            "supersededV2SchemaSha256": sha256_bytes((V2_ROOT / "preflight_v2.schema.json").read_bytes()),
            "supersededV2DigestFileSha256": sha256_bytes((V2_ROOT / "fixtures" / "DIGEST").read_bytes()),
        ...
        "verification": {
            ...
            "refusalUnknownFieldsRejectedBySchema": tamper["refusalUnknownFieldsRejectedBySchema"],
            "refusalEnvelopeInjectionsRejectedByConsumer": tamper["refusalEnvelopeInjectionsRejectedByConsumer"],
            "v1FixturesRejectedByV3Schema": rejected_v1,
            "v2FixturesRejectedByV3Schema": rejected_v2,
        },
        "findings": findings,
        ...
        "recommendation": {
            ...
            "nextGate": (
                ...
                if passed
                else "correct the v3 contract at a new immutable version boundary, then rerun this audit"
            ),
        },
    }
