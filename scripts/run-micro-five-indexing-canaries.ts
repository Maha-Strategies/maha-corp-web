...
export const CONFIRMATION = 'PUBLISHER_FUNDED_MICRO_FIVE_ONCE_MAX_0_03_USDC'
export const TARGETS = [
  { id: 'citation-binding-check', path: '/api/v1/micro/citation-binding-check', amount: '5000' },
  { id: 'revision-lineage-check', path: '/api/v1/micro/revision-lineage-check', amount: '5000' },
  { id: 'audit-export-normalizer', path: '/api/v1/micro/audit-export-normalizer', amount: '10000' },
  { id: 'unit-uncertainty-conversion', path: '/api/v1/micro/unit-uncertainty-conversion', amount: '5000' },
  { id: 'divine-name-disambiguation', path: '/api/v1/micro/divine-name-disambiguation', amount: '5000' },
] as const
...
async function run() {
  if (process.env.MICRO_CANARY_CONFIRMATION !== CONFIRMATION || process.env.GITHUB_RUN_ATTEMPT !== '1') throw new Error('explicit_authorization_and_first_attempt_required')
  const outputPath = process.env.MICRO_CANARY_OUTPUT_PATH
  ...
  if (balance < BigInt(30000)) throw new Error('insufficient_balance_for_authorized_plan')
  const evidence = { version: 'maha-micro-five-indexing-canary/0.1', classification: 'publisher-funded-indexing-canary',
    customerDemand: false, organicDemand: false, maximumAuthorizedBaseUnits: '30000', confirmedBaseUnits: '0',
