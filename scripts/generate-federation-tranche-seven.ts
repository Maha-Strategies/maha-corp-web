...
export function generateTrancheSeven(outputRoot = ROOT) {
  ...
  const priorCohorts = [1, 2, 3, 4, 5, 6].map((tranche) => readJson<Parameters<typeof selectFederationTranche>[4][number]>(`content/federation/federation-tranche-${tranche}-cohort-v1.json`))
  ...
  const cohort = selectFederationTranche(candidateMap, semantic, graph, demand, priorCohorts, 7)
  const output = resolve(outputRoot, 'content/federation/federation-tranche-7-cohort-v1.json')
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  const cohort = generateTrancheSeven(resolve(argument('output-root') ?? ROOT))
