...
const ARGS = ['-h', '/private/tmp', '-p', '55444', '-U', 'postgres', '-tAc']
...
/**
 * Membership spans two batches, and a record joins only once.
 *
 * Batch 3 alone had three accepted proposals and Batch 4 two distinct records,
 * which is why the canary is possible now and was not before. Five distinct
 * records or no canary: a four-record run is not a five-record canary with one
 * missing.
 */
type Entry = (typeof packets3.ledgerEntries)[number]
const all = [...(packets3.ledgerEntries as Entry[]), ...(packets4.ledgerEntries as unknown as Entry[])]
  .filter((e) => e.proposedDisposition === 'accept')
const seen = new Set<string>()
const entries = all.filter((e) => {
  if (seen.has(e.recordIdentity.route)) return false
  seen.add(e.recordIdentity.route)
  return true
}).slice(0, 5)
if (entries.length < 5) {
  console.log(JSON.stringify({ refused: 'fewer than five distinct accepted records', available: entries.length }, null, 2))
  process.exit(0)
}
...
const report = {
  schemaVersion: 'maha-cross-batch-source-override-canary/1.0',
  batchesSpanned: ['evidence-recovery-3', 'source-acquisition-4'],
  membershipRule: 'Five distinct accepted records across the two batches, each joining once. Fewer than five refuses rather than shrinking.',
  ranAt: '2026-09-02',
  environment: {
...
writeFileSync('content/evidence-batch-4/cross-batch-canary-evidence.json', `${JSON.stringify(report, null, 2)}\n`)
