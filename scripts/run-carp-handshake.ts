...
async function main() {
  ...
  const requestId = `maha-handshake-${Date.now()}`
  ...
  const evidence = {
    ...
    checks: {
      publishedDidMatchesSad: true,
      sadSignatureVerified: true,
      elCabezonChallengeAnswered: true,
      elCabezonAcknowledgedMahaKey: true,
      encryptedAboutAccepted: true,
      asynchronousResultPending: true,
    },
    requestId,
    caveats: [
