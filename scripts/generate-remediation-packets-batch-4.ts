...
import batch2 from '../content/evidence-batch-4/inspections.json' with { type: 'json' }
...
type B4 = Insp & { claimByClaimSupport: { route: string; locator: string; claim: string }[]; class: string }
const packets = (batch2.inspected as unknown as B4[]).flatMap((source) =>
  source.claimByClaimSupport.map(({ route, locator, claim }) => {
...
writeFileSync('content/evidence-batch-4/remediation-packets.json', `${JSON.stringify(ledger, null, 2)}\n`)
