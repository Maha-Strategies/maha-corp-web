...
export function OpenBookSectionReader({ book, section, markdown }: { book: OpenBookEdition; section: OpenBookSection; markdown: string }) {
  return <main className="min-h-screen bg-[#0a0a0c] text-zinc-300 selection:bg-indigo-500 selection:text-white"><article className="max-w-3xl mx-auto px-6 py-20 sm:py-28"><Link href={`/books/${book.slug}/read`} className="inline-block font-mono text-xs text-indigo-300 hover:text-white tracking-widest uppercase transition-colors mb-12">← All chapters</Link><header className="border-b border-zinc-800 pb-10 mb-12"><p className="font-mono text-xs text-indigo-300 tracking-widest uppercase mb-5">[ Open edition ]</p><h1 className="text-4xl sm:text-5xl font-light text-white leading-[1.1] tracking-tight mb-5">{section.title}</h1><p className="text-lg text-zinc-400 font-light leading-relaxed">A section of {book.title} by Mayone Maha Rajan.</p></header><BookManuscript markdown={markdown} /><OpenBookSectionNavigation book={book} section={section} /><footer className="mt-16 border-t border-zinc-800 pt-8"><Link href={`/books/${book.slug}`} className="text-sm text-zinc-300 hover:text-white">Return to the book’s table of contents ↗</Link></footer></article></main>
}
