...
export function LogAuditUpload() {
  ...
  return <section className="mt-8 border border-indigo-900/70 bg-indigo-950/10 p-6" aria-labelledby="log-audit-heading">
    <p className="font-mono text-[10px] uppercase tracking-widest text-indigo-300">Enterprise ROI auditor / browser-local</p>
    <h2 id="log-audit-heading" className="mt-3 text-2xl font-light text-white">Audit a request-log export without uploading it.</h2>
    <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-400">Drop an OpenAI-style JSONL export or LangChain/LangSmith JSON trace. Parsing and estimates stay in this browser. Select how many months the file represents to normalize the dashboard.</p>
    <div className="mt-5 flex flex-wrap items-end gap-4"><label className="text-sm text-zinc-300">Months represented<input type="number" min="1" max="120" value={months} onChange={(event) => setMonths(Math.max(1, Math.min(120, Number(event.target.value) || 1)))} className="ml-3 w-20 border border-zinc-700 bg-black px-2 py-2 text-sm text-white outline-none focus:border-indigo-300" /></label></div>
    ...
    <button type="button" onClick={chooseFile} onDragOver={(event) => event.preventDefault()} onDrop={drop} className="mt-6 flex min-h-32 w-full items-center justify-center border border-dashed border-indigo-700 bg-black/30 px-5 text-center text-sm text-indigo-100 hover:border-indigo-300">Drop .jsonl / .json here, or choose a local file</button>
    {error && <p className="mt-4 text-sm text-red-300">{error}</p>}
    {result && <div className="mt-7"><p className="text-xs text-zinc-500">{number.format(result.requestCount)} request records · {number.format(result.tokens)} estimated input tokens/month · illustrative GPT-4o-style input rate</p><div className="mt-4 grid gap-3 md:grid-cols-3"><article className="border border-zinc-800 bg-black/30 p-4"><p className="font-mono text-[10px] uppercase text-zinc-500">Raw historical spend</p><p className="mt-2 text-2xl text-white">{money.format(result.rawMonthlySpend)} <span className="text-sm text-zinc-500">/ mo</span></p></article><article className="border border-zinc-800 bg-black/30 p-4"><p className="font-mono text-[10px] uppercase text-zinc-500">Compressed spend with Maha</p><p className="mt-2 text-2xl text-emerald-200">{money.format(result.compressedMonthlySpend)} <span className="text-sm text-zinc-500">/ mo</span></p></article><article className="border border-emerald-900 bg-emerald-950/20 p-4"><p className="font-mono text-[10px] uppercase text-emerald-300">Net savings & ROI</p><p className="mt-2 text-xl text-white">Saved {money.format(result.monthlySavings)} / mo</p><p className="mt-1 text-xs text-emerald-200">{result.pilotRoi.toFixed(0)}% first-year ROI on a $5k pilot</p></article></div><p className="mt-4 text-xs leading-5 text-zinc-500">Planning estimate only: a 55% input-token reduction scenario. Validate task quality, output-token spend, provider pricing, and log period before committing budget.</p><button type="button" onClick={() => setModalOpen(true)} className="mt-6 bg-indigo-300 px-5 py-3 font-mono text-xs font-bold uppercase tracking-widest text-black hover:bg-indigo-200">Claim your $5,000 guaranteed enterprise pilot key →</button></div>}
    {modalOpen && <div role="dialog" aria-modal="true" aria-label="Enterprise pilot inquiry" className="mt-6 border border-indigo-500 bg-[#0b0d14] p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="text-lg text-white">Enterprise pilot inquiry</h3><p className="mt-2 text-sm leading-6 text-zinc-400">Your audit file stays local. Submit only the contact details below through your email client.</p></div><button type="button" onClick={() => setModalOpen(false)} className="text-zinc-400 hover:text-white" aria-label="Close">×</button></div>{submitted ? <p className="mt-5 text-sm text-emerald-200">Your email client was opened with the inquiry details. Send it when ready.</p> : <form onSubmit={submitPilot} className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm text-zinc-300">Company email<input name="email" type="email" required className="mt-2 w-full border border-zinc-700 bg-black px-3 py-2 text-white outline-none focus:border-indigo-300" /></label><label className="text-sm text-zinc-300">Monthly token spend<input name="monthlySpend" required defaultValue={result ? money.format(result.rawMonthlySpend) : ''} className="mt-2 w-full border border-zinc-700 bg-black px-3 py-2 text-white outline-none focus:border-indigo-300" /></label><button className="bg-indigo-300 px-4 py-3 font-mono text-xs font-bold uppercase tracking-widest text-black sm:col-span-2">Prepare opted-in inquiry →</button></form>}</div>}
  </section>
}
