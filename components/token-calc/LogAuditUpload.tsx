...
export function LogAuditUpload() {
  ...
  return <section className="mt-8 border border-[var(--status-sourced)] bg-[var(--surface-raised)] p-6" aria-labelledby="log-audit-heading">
    <p className="font-mono text-[10px] uppercase tracking-widest text-[var(--text-muted)]">Enterprise ROI auditor / browser-local</p>
    ...
    <div className="mt-5 flex flex-wrap items-end gap-4"><label className="text-sm text-[var(--text-secondary)]">Months represented<input type="number" min="1" max="120" value={months} onChange={(event) => setMonths(Math.max(1, Math.min(120, Number(event.target.value) || 1)))} className="evidence-input ml-3 w-20" /></label></div>
    ...
    <button type="button" onClick={chooseFile} onDragOver={(event) => event.preventDefault()} onDrop={drop} className="mt-6 flex min-h-32 w-full items-center justify-center border border-dashed border-[var(--status-sourced)] bg-[var(--surface-raised)] px-5 text-center text-sm text-[var(--text-primary)] hover:border-[var(--status-sourced)]">Drop .jsonl / .json here, or choose a local file</button>
    {error && <p className="mt-4 text-sm text-[var(--text-secondary)]">{error}</p>}
    {result && <div className="mt-7"><p className="text-xs text-[var(--text-muted)]">{number.format(result.requestCount)} request records · {number.format(result.tokens)} estimated input tokens/month · illustrative GPT-4o-style input rate</p><div className="mt-4 grid gap-3 md:grid-cols-3"><article className="border border-[var(--border-default)] bg-[var(--surface-raised)] p-4"><p className="font-mono text-[10px] uppercase text-[var(--text-muted)]">Raw historical spend</p><p className="mt-2 text-2xl text-[var(--text-primary)]">{money.format(result.rawMonthlySpend)} <span className="text-sm text-[var(--text-muted)]">/ mo</span></p></article><article className="border border-[var(--border-default)] bg-[var(--surface-raised)] p-4"><p className="font-mono text-[10px] uppercase text-[var(--text-muted)]">Compressed spend with Maha</p><p className="mt-2 text-2xl text-[var(--status-verified)]">{money.format(result.compressedMonthlySpend)} <span className="text-sm text-[var(--text-muted)]">/ mo</span></p></article><article className="border border-[var(--status-verified)] bg-[var(--surface-raised)] p-4"><p className="font-mono text-[10px] uppercase text-[var(--status-verified)]">Net savings & ROI</p><p className="mt-2 text-xl text-[var(--text-primary)]">Saved {money.format(result.monthlySavings)} / mo</p><p className="mt-1 text-xs text-[var(--status-verified)]">{result.pilotRoi.toFixed(0)}% first-year ROI on a $5k pilot</p></article></div><p className="mt-4 text-xs leading-5 text-[var(--text-muted)]">Planning estimate only: a 55% input-token reduction scenario. Validate task quality, output-token spend, provider pricing, and log period before committing budget.</p><button type="button" onClick={() => setModalOpen(true)} className="evidence-action evidence-action--primary mt-6">Claim your $5,000 guaranteed enterprise pilot key →</button></div>}
    {modalOpen && <div role="dialog" aria-modal="true" aria-label="Enterprise pilot inquiry" className="mt-6 border border-[var(--status-sourced)] bg-[var(--surface-raised)] p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="text-lg text-[var(--text-primary)]">Enterprise pilot inquiry</h3><p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">Your audit file stays local. Submit only the contact details below through your email client.</p></div><button type="button" onClick={() => setModalOpen(false)} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]" aria-label="Close">×</button></div>{submitted ? <p className="mt-5 text-sm text-[var(--text-secondary)]">Your email client was opened with the inquiry details. Send it when ready.</p> : <form onSubmit={submitPilot} className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm text-[var(--text-secondary)]">Company email<input name="email" type="email" required className="evidence-input mt-2" /></label><label className="text-sm text-[var(--text-secondary)]">Monthly token spend<input name="monthlySpend" required defaultValue={result ? money.format(result.rawMonthlySpend) : ''} className="evidence-input mt-2" /></label><button className="evidence-action evidence-action--primary">Prepare opted-in inquiry →</button></form>}</div>}
  </section>
}
