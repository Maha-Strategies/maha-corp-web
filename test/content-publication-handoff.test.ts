...
const candidate = { public_id: 'contentcand_1234567890abcdef1234567890abcdef', proposed_path: '/mps/audit-ai-writing-before-publishing', quality_score: 90, evidence: [
  { url: 'https://example.gov/standard', title: 'Official claim verification standard', note: 'Defines the review boundary and the evidence record required for a claim.' },
  { url: 'https://example.edu/research', title: 'Independent research on citation reliability', note: 'Documents the observed citation-reliability problem that this workflow addresses.' },
  { url: 'https://example.org/guidance', title: 'Editorial guidance for source attribution', note: 'Explains how editors should distinguish attribution from independent verification.' },
], policy_checks: { readerFirst: true, originalAnalysis: true, notDoorway: true, attributionComplete: true, sourceIndependenceReviewed: true, humanReviewRequired: true }, status: 'approved_for_draft' }
...
test('placeholder source metadata is a hard publication blocker', () => {
  const result = publicationHandoff({ candidate: { ...candidate, evidence: [{ url: 'https://example.gov/one', title: 'Source one', note: 'What this source establishes for the reader.' }, ...candidate.evidence.slice(1)] }, draft })
  assert.equal(result.checklist.sourceMetadataComplete, false)
  assert.equal(result.decision, 'withheld')
})
