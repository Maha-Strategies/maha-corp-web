...
test('the route ledger accounts for the KDP withdrawal in its prepared projection', () => {
  ...
  assert.equal(scaling.localUnpublishedTranche.grossAddedCrawlableSurfaces, 250)
  assert.equal(scaling.localUnpublishedTranche.mahaPrincipleKdpBoundary.withdrawnCanonicalSurfaces, 42)
  assert.equal(scaling.localUnpublishedTranche.totalCrawlableSurfaces, 208)
  assert.equal(scaling.localUnpublishedTranche.projectedRoutesAfterOneDeployment, 1003)
  assert.equal(scaling.localUnpublishedTranche.projectedGapToTarget, 0)
  assert.equal(scaling.localUnpublishedTranche.projectedRoutesBeyondTarget, 3)
