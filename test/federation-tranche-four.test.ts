...
test('local implementation and owner adapters remain unreleased and preserve the one genuine dependency hold', () => {
  assert.deepEqual(implementationRegistry.counts, {
    pages: 366,
    readyForOwnerIntegration: 365,
    blockedOnUnreadyPrerequisite: 1,
    byProperty: { 'maha-strategies': 108, 'maha-research': 100, 'agentic-publishing': 26, 'maha-os': 24, 'mayone-maharajan': 16, 'mayon-rajan': 20, 'maha-policy': 72 },
    boundedAnswers: 1830,
    sourceBindings: 648,
  ...
  assert.equal(blocked[0]!.canonicalUrl, 'https://publish.mahastrategies.com/agentic-publishing/machine-readable-article/governance')
  assert.deepEqual(adapterRegistry.counts, { properties: 7, routes: 365, boundedAnswers: 1825, publicRoutesCreated: 0, buildsRun: 0 })
