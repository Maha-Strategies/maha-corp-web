...
test('adapter output hashes are deterministic while ingestion batches remain append-only events', () => {
  ...
  const expectedRecordCount = adapter.adapt().length
  assert.equal(first.recordCount, expectedRecordCount)
  ...
  assert.equal(ingestionBatchSnapshot(first).records.length, expectedRecordCount)
