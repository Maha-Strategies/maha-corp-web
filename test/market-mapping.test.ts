...
const input = {
  source: 'outbound_scout', signalClass: 'buyer_demand', sourceReference: 'scout:2026-07-21:receipt-ops', title: 'Receipt-expense CSV gap for small operators',
...
test('market opportunities require attributable HTTPS evidence and score deterministically', () => {
  ...
  assert.throws(() => parseMarketOpportunity({ ...input, signalClass: 'unknown' }), /signalClass is not supported/)
})
