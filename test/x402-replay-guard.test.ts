...
import { createReplayGuard, type ClaimedPayment } from '../lib/x402/replay-guard.ts'
...
const payment: ClaimedPayment = {
  transaction: 'tx_1', network: 'eip155:8453', payer: '0xAgent', amountPaid: '10000', asset: '0xUSDC',
}
...
test('a first presentation is claimed', async () => {
  const guard = createReplayGuard(ledger({ data: 'claimed' }), payment, requirement)
  assert.equal(await guard.claim('tx_1'), true)
})

test('a repeat presentation is refused', async () => {
  const guard = createReplayGuard(ledger({ data: 'duplicate' }), payment, requirement)
  assert.equal(await guard.claim('tx_1'), false)
})
...
test('the claim records the resource that was actually paid for', async () => {
  ...
  const guard = createReplayGuard(ledger({ data: 'claimed' }, calls), payment, requirement)
  await guard.claim('tx_1')
...
test('a database failure withholds the resource rather than serving it unrecorded', async () => {
  ...
  for (const error of [{ code: 'PGRST202' }, { code: '42501' }, {}]) {
    const guard = createReplayGuard(ledger({ error }), payment, requirement)
    assert.equal(await guard.claim('tx_1'), false)
  }
})
...
test('an unexpected return value is treated as unclaimed', async () => {
  for (const data of [null, undefined, '', 'unexpected', 0]) {
    const guard = createReplayGuard(ledger({ data }), payment, requirement)
    assert.equal(await guard.claim('tx_1'), false)
  }
})
