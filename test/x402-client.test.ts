...
const REQUIREMENT: PaymentRequirement = {
  ...
  resource: 'https://maha.test/api/v1/compress',
  description: 'One context compression',
...
const challengeHeader = (accepts: PaymentRequirement[] = [REQUIREMENT], error = 'Payment is required for this resource.') =>
  toBase64({ x402Version: 1, accepts, error })
...
test('a requirement is only selected for the chain the wallet is on', () => {
  ...
  assert.equal(selectRequirement(challenge, 84532)?.network, 'base-sepolia')
...
test('a challenge that cannot be read is a clear error, not a crash', () => {
  ...
  assert.throws(() => decodeChallenge(toBase64({ x402Version: 1, accepts: [] })), /no terms/)
})
...
test('the header the client builds hashes to the id the server claims against', async () => {
  ...
  const header = encodePaymentSignature(REQUIREMENT, typedData.message, SIGNATURE)
  const serverSide = JSON.parse(Buffer.from(header, 'base64').toString('utf8'))

  assert.equal(serverSide.x402Version, 1)
  assert.equal(serverSide.scheme, 'exact')
  assert.equal(serverSide.network, 'base-sepolia')
...
test('amounts are shown to people in the asset, not in base units', () => {
  ...
  assert.equal(formatAmount({ ...REQUIREMENT, maxAmountRequired: '1500000' }), '1.5 USDC')
  assert.equal(formatAmount({ ...REQUIREMENT, maxAmountRequired: '2000000' }), '2 USDC')
})
