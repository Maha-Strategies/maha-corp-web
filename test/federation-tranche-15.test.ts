...
const read = (n: string) => JSON.parse(readFileSync(`${F}/federation-tranche-15-${n}-v1.json`, 'utf8'))
...
const report = readFileSync('docs/operations/federation-tranche-15-readiness.md', 'utf8')
...
test('projected unlocks equal the fan-out of the prerequisites selected', () => {
  ...
  assert.equal(cohort.dependencyFirst.projectedTranche14Unlocks,
...
test('a definition supplied inside this cohort resolves its dependents', () => {
  ...
  const present = (deps.dependencies as { state: string }[]).filter((d) => d.state === 'present-in-tranche-15')
...
test('the Markdown report agrees with the artifacts on every count', () => {
  ...
  for (const [label, value] of [
    ...
    ['Projected Tranche 14 unlocks', c.projectedTranche14Unlocks],
  ] as [string, number][]) {
...
test('artifacts and report regenerate byte-identically', () => {
  ...
  ].map((n) => `${F}/federation-tranche-15-${n}-v1.json`)
  const before = [...paths.map((p) => readFileSync(p, 'utf8')), report]
  execFileSync('node', ['--experimental-strip-types', 'scripts/generate-federation-tranche-15.ts'], { stdio: 'ignore' })
  const after = [...paths.map((p) => readFileSync(p, 'utf8')),
    readFileSync('docs/operations/federation-tranche-15-readiness.md', 'utf8')]
